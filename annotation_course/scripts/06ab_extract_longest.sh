...
samtools faidx "$protein" $(cat protein_longest_ids.txt | tr '\n' ' ') > ERR11437317.asm.bp.p_ctg.all.maker.proteins.renamed.filtered.longest2.fasta
...
# Step 3: Extract sequences using samtools
samtools faidx "$transcript" $(cat transcript_longest_ids.txt | tr '\n' ' ') > ERR11437317.asm.bp.p_ctg.all.maker.transcripts.renamed.filtered.longest2.fasta
